    ...
    const analyze = () => {
      ...
      if (analyserRef.current && dataArrayRef.current) {
        ...
        if (autoEq) {
          ...
          // Bands for 512 FFT (approx 86Hz per bin)
          // Low: 20-250Hz (bins 0-3), Mid: 250-4kHz (bins 3-46), High: 4k-20kHz (bins 46-232)
          let lSum = 0; for (let i = 0; i < 3; i++) lSum += data[i];
          let mSum = 0; for (let i = 3; i < 46; i++) mSum += data[i];
          let hSum = 0; for (let i = 46; i < 232; i++) hSum += data[i];
          
          const lAvg = lSum / 3;
          const mAvg = mSum / 43;
          const hAvg = hSum / 186;
          const median = (lAvg + mAvg + hAvg) / 3;
          
          // 6dB threshold is approx 22 units in byte frequency data (-100 to -30 range)
          const threshold = 22;
          if (lAvg > median + threshold) {
            // Dynamically attenuate bass shelf (first filter)
            const diff = lAvg - (median + threshold);
            const attenuation = Math.min(12, diff * 0.5); // Cap at 12dB attenuation
            autoEqBassRef.current += (attenuation - autoEqBassRef.current) * 0.1;
          } else {
            autoEqBassRef.current *= 0.95; // Gradually return to normal
          }
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        g.addColorStop(0,   hexToRgba(moodColor, al));
        g.addColorStop(0.4, hexToRgba(moodColor, al * 0.6));
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      if (ambientStyle === "BREATHE") {
        ...
        const coreGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, W * (0.3 + pulse * 0.2));
        coreGrad.addColorStop(0, hexToRgba(moodColor, 0.05 + pulse * 0.1));
  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 flex items-center justify-center h-[100dvh] w-full p-2 md:p-6 overflow-hidden">
        ...
        >
          {/* Header Stats - Now using absolute but within a constrained container */}
          <div className="absolute top-4 md:top-6 left-4 md:left-12 right-4 md:right-12 flex justify-between items-center z-20">
          ...
          {/* Left: Visualizer Disc */}
          <div className="flex flex-col items-center justify-center flex-shrink-0 mt-8 md:mt-0">
            <div 
              onTouchStart={startTouchDrag}
              onTouchMove={onTouchMove}
              onTouchEnd={onTouchEnd}
              className="relative w-[160px] h-[160px] sm:w-[220px] sm:h-[220px] md:w-80 md:h-80"
            >
              <motion.div 
                key={tidx}
          ...
          {/* Right: Controls & Data */}
          <div className="flex flex-col items-center text-center md:items-start md:text-left justify-center w-full md:w-80 min-h-0 md:overflow-hidden pb-8 md:pb-0">
            
            {/* Static Metadata Block */}
            <div className="h-20 md:h-28 w-full flex flex-col items-center justify-center bg-white/[0.03] border border-white/10 rounded-2xl mb-3 md:mb-4 flex-shrink-0 overflow-hidden relative group">
               ...
               <AnimatePresence mode="wait">
                 ...
                 >
                   <h2 className="text-white text-xl md:text-2xl font-black tracking-tighter uppercase leading-tight mb-1 truncate">
                   ...
                   <p className="text-emerald-400 text-[9px] font-bold tracking-[0.3em] uppercase opacity-80 truncate">
            ...
            {/* Frequency Bars - Positioned between Metadata and Moods */}
            <div className="h-8 md:h-12 w-full bg-white/[0.02] rounded-lg overflow-hidden border border-white/5 mb-4 md:mb-6 opacity-80 flex-shrink-0">
